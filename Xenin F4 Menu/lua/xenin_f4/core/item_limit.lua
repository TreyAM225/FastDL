local function Override()
	...
	if (SERVER) then
		-- https://github.com/FPtje/DarkRP/blob/7f40e23d283a95fea557b41c56016acc10dec8cd/gamemode/modules/base/sv_entityvars.lua#L220-L253
	...
	if (CLIENT) then
		net.Receive("F4Menu.SyncEntLimit", function(len)
			...
			if (increment) then
				ply:addCustomEntity({ cmd = cmd })
			else
				ply:removeCustomEntity({ cmd = cmd })
			end
		end)
	end
