...
function PANEL:CreateCommands()
	...
	for k, cat in ipairs(F4Menu.Commands) do
		...
		for i, v in  pairs(cat.commands) do
			...
			panel:SetText("")
			panel.Background = Color(0, 0, 0, 100)
			...
			panel.OnCursorEntered = function(pnl)
				pnl:LerpColor("Background", Color(0, 0, 0, 200))
				pnl:LerpColor("TextColor", color_white)
			end
			panel.OnCursorExited = function(pnl)
				pnl:LerpColor("Background", Color(0, 0, 0, 100))
			...
			panel.DoClick = function(pnl)
				if (v.input) then
					...
					F4Menu:Modal(translate(v.name) or "", 
						translate(input.desc or v.name) or "",
						translate(isfunction(input.str) and input.str(ply) or input.str or "") or "",
						translate(isfunction(input.placeholder) and input.placeholder(ply) or input.placeholder or "") or "",
						input.numeric,
						function(value, pnl, sid64)
