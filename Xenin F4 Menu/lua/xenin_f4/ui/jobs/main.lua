...
function PANEL:Build()
	...
	if (showFavourites) then
		...
		table.insert(buildTree, {
			...
			hook = function(self, cat)
				cat:AddHook("F4Menu.FavouriteAdded", "F4Menu.FavouriteJobs", function(pnl, data)
					...
					-- We need to make sure it exists, error will happen here
	...
	for i, v in ipairs(cats) do
		...
		for i, v in ipairs(members) do
			if (v.NeedToChangeFrom and v.NeedToChangeFrom != plyTeam) then continue end
	...
	for i, v in ipairs(buildTree) do
		...
		cat:SetName(v.name)
		local w = 76 * F4Menu.Config.ColumnsPerRow

		local content = self:Sort(v.content, sort)
		cat:SetInstantExpandWidth(w)
		cat:FeedData(content, 1, F4Menu.Config.CategoriesStartExpanded and #v.content > 0)

		-- Some custom code for this, cuz why not!
		if (v.hook) then
			v:hook(cat)
		end
	end
...
function PANEL:Init()
	...
	self.Top.Favourite.OnStateChanged = function() self:Build() end
	...
	self.Top.Recent:SetState(true, true)
	self.Top.Recent.OnStateChanged = function() self:Build() end

  self.Top.Sort = self.Top:Add("DButton")
	self.Top.Sort:Dock(LEFT)
	self.Top.Sort:DockMargin(8, 0, 0, 0)
  self.Top.Sort:SetText(F4Menu:GetPhrase("jobs.top.sort.default"))
  ...
  self.Top.Sort.DoClick = function(pnl)
    ...
    panel:AddChoice(F4Menu:GetPhrase("jobs.top.sort.default"), func, nil, hoverColor)
    panel:AddChoice(F4Menu:GetPhrase("jobs.top.sort.alphabetically"), func, nil, hoverColor)
		panel:AddChoice(F4Menu:GetPhrase("jobs.top.sort.highestSalary"), func, nil, hoverColor)
		panel:AddChoice(F4Menu:GetPhrase("jobs.top.sort.lowestSalary"), func, nil, hoverColor)
		panel:AddChoice(F4Menu:GetPhrase("jobs.top.sort.mostPeople"), func, nil, hoverColor)
		panel:AddChoice(F4Menu:GetPhrase("jobs.top.sort.leastPeople"), func, nil, hoverColor)
  end
...
function PANEL:CreateOverlay(team)
	...
	self.Overlay:AddHook("F4Menu.CloseMenu", "Slideout", function(pnl)
		pnl:Remove()
	end)
