...
function PANEL:Init()
  ...
  self.Favourites.Layout.PerformLayout = function(pnl, w, h)
	...
	self.Favourites.Layout:AddHook("F4Menu.FavouriteAdded", "F4Menu.Dashboard.FavouriteJobsAdded", function(pnl, data)
	...
	self.Favourites.Layout:AddHook("F4Menu.FavouriteRemoved", "F4Menu.Dashboard.FavouriteJobs", function(pnl, name)
	...
	self.Navbar:AddTab(F4Menu:GetPhrase("dashboard.tabs.general.name"), "F4Menu.Dashboard.Stats.General")
	self.Navbar:AddTab(F4Menu:GetPhrase("dashboard.tabs.economy.name"), "F4Menu.Dashboard.Stats.Graph")
...
function PANEL:CreateOverlay(team)
	...
	self.Overlay:AddHook("F4Menu.CloseMenu", "Slideout.Dashboard", function(pnl, dashboard)
		if (dashboard) then
			pnl:Remove()
		end
	end)
