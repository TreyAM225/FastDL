...
function PANEL:Build()
	...
	self:AddHook("OnPlayerChangedTeam", "F4Menu.Items." .. name, function(pnl, ply)
		pnl.Rebuild = true
	end)
	...
	if (showFavourites) then
		...
		table.insert(buildTree, {
			...
			hook = function(self, cat)
				cat:AddHook("F4Menu.Items.FavouriteAdded." .. name, "F4Menu.FavouriteJobs", function(pnl, data)
