...
onyx.inconfig.index = onyx.inconfig.values or {}
...
function inconfig:Register(id, data)
    ...
    assert(isstring(data.cami), Format('bad field \"cami\"\'s value in option \"%s\" (expected string, got %s)', id, type(data.cami)))                                                                                                                                             -- f57a421c-13b9-4a6d-beaf-215b44fe1613 | edfe01e2-3123-4d95-a7f6-ded84410734c
    data.id = id
    if (SERVER) then
        data.title = nil
        data.desc = nil
...
if (SERVER) then
    ...
    net.Receive('onyx.inconfig:SyncSingle', function()
        ...
        hook.Run('onyx.inconfig.Updated')
    end)
end